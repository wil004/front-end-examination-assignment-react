...
function Register() {
    ...
    useEffect(() => {
        emailIsValid(emailAdress, setIsEmailValid);
    ...
    return (
        <main className="register">
            {loading ? <p>Loading please wait...</p> : registerSucces ?
                <div>
                    <h2 className="registerSucces">Registration Successful! Redirecting in {count}</h2>
                ...
                <section>
                    ...
                    <form className="registerForm">
                        {error ? <p className="registerInfo">{errorCode}</p> :
                            <p className="registerInfo">Create an account here!</p>}
                        <label htmlFor="email">Fill in your email-adress</label>
                        <input type="text" id="email" value={emailAdress} onChange={(e) => {
                            setEmailAdress(e.target.value)
                        }}/>
                        {emailAdress.length > 0 && !isEmailValid &&
                        <p className="validity">Email-adress must contain an @ and dot</p>}
                        <label htmlFor="userName">Choose a username</label>
                        <input type="text" id="userName" value={userName} onChange={(e) => {
                            setUserName(e.target.value)
                        }}/>
                        {userName.length > 0 && userName.length < 6 &&
                        <p className="validity">Username requires at least 6 characters.</p>}
                        <label htmlFor="password1">Choose password</label>
                        <input type="password" id="password1" value={password1} onChange={(e) => {
                            setPassword1(e.target.value)
                        }}/>
                        {password1.length > 0 && password1.length < 6 &&
                        <p className="validity">Password should contain at least 6 characters</p>}
                        ...
                        <p className="validity">Passwords must be equal!</p>}
                        <button
                            ...
                            disabled={registerButton}>Register
                        </button>
                    </form>
                    <p>If you already have an account you can sign in <NavLink to="/login" className="registerUrl">here</NavLink>.</p>
                </section>}
        </main>
    );
}
