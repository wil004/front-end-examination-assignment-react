...
function ChangeProfile ({title, description, urlText, url, inputFieldDescription, currentDetails, currentDetailsTitle}) {
    ...
    useEffect(() => {emailIsValid(inputSetter, setIsEmailValid);}, [inputSetter]);
    ...
    <section className="personalPageSetImage">
        <div>
            ...
            <img src={auth.user.profilePicture ? auth.user.profilePicture : defaultProfilePicture} className="personalPageProfile"/>
            {currentDetails}
        </div>
        ...
        <section className="setImageInput">
            ...
            />
                ...
                />
                    {inputSetter !== password2 && <p className="changeProfileValidity">Passwords must be equal</p>}
                </div>    }
                {setAccount === "changeEmail" && !isEmailValid && inputSetter.length > 0 && <p className="changeProfileValidity">Email-address must contain an @ and dot</p>}
            </div>
            <div className="setNewPersonalData">
                ...
                <button className="personalPageButtonImage" onClick={() => {currentProfile(); setAccount === "setImage" ?
                    setImage(inputSetter) : setAccount === "changeEmail" ? setEmail(inputSetter) :
                        setAccount === "changePassword" && setPassword(inputSetter);}}
                disabled={setAccount === "changePassword" ? inputSetter !== password2 : !isEmailValid}>Confirm</button>
